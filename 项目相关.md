...
用到了智能指针吗 如何选择指针          ev项目

用到了多线程吗 如何同步 如何debug

为什么多线程，什么时候多线程什么时候多进程
...
goa-json中自定义了可变类型数据存储类`Value`，其设计类似于QT中的QVariant和C++17中的Variant，json文档中的每一个元素（DOM树状结构上的叶子节点或子树）都可以用`Value`来存储，是DOM API的核心。其内部维护一个union来存储数据并节约存储空间，如下：
...
在document一边，基于value实现了handler的接口。其核心函数是addValue，通过维护一个栈，在构建array和object开始时，startxxx会构造一个Value入栈，这个value就是array和object对应的父value，之后继续解析，并向栈中的父Value其中添加单个或者kv类型的子Value数据，构建完毕后，由endxxx进行出栈。最终返回一个完整了树状结构。
...
1. std::enable_if_t    is_same_v（c++17)

在实现Reader时，需要实现parse函数从输入流进行解析，该函数为模板函数。要求输入流只能是已经实现的FileReadStream和StringReadStream。

使用std::enable_if_t和std::is_same来限制模板参数

2. **`std::string_view`**
   - 一个指向不可变字符序列的非拥有引用。它包含一个指针和一个长度，可以高效地“视图”任何类型的字符串数据
...
每个TcpServerSingle都有相同的Acceptor，监听相同的端口，交给epoll监听，并通过newConnectionCallback_处理新的连接，newConnectionCallback会建立Tcpconnection实例，并设置Tcpconnection的回调。Tcpconnection在初始化时会设置自己对应的channel的回调，实际是将上层传递给TcpConnection的回调继续传递给channel。
 ...
 **I/O 多路复用监听事件，收到事件后，根据事件类型分配（Dispatch）给某个进程 / 线程去处理**。
...
std::format  使用 `{}`作为占位符，并在运行时用提供的参数替换它们  ，类似python

 vformat结合make_format_args(),   可接受格式字符串和可变参数

std::osyncstream 线程安全的输出  （std::cout是不安全的）

##### 用到了哪些智能指针

* unique_ptr

	1. 主server管理子server，子server运行在不同的线程中，主server管理这些线程。独占子server线程的所有权
	1. 线程池管理计算线程，独占其所有权
	1. 客户端一边，每个client独占一个Connector

* shared_ptr  weak_ptr

1. server需要管理connection，而每个channel需要绑定一个connection，即二者同时拥有connection。
...
# 为特定线程设置条件断点
break <location> thread <thread_id> if <condition>
```
...
- 主要时间在等待I/O操作完成
- CPU使用率较低（经常处于空闲状态）
- 有大量的阻塞时间： 从磁盘、网络中读取数据到内存
  * 例子 ： 网络请求（HTTP API调用、数据库查询）       文件读写（磁盘I/O）      数据库操作         用户输入处理

CPU密集型：

- 主要消耗CPU计算资源
- 大部分时间在执行数学运算、逻辑判断、数据处理
- CPU使用率接近100%
- 对内存访问频繁，但等待时间短
...
2. 基于线程数计算公式，理想线程数 = CPU核心数 × (1 + I/O等待时间比例)

   IO占比时间越多，线程就可以越多

3. **考虑约束条件**：内存、数据库连接、系统负载等

   **内存限制**：每个线程都会占用一定的内存（主要是栈空间，默认大小可能为8MB，但可以调整）。如果线程数过多，可能会耗尽内存，导致系统性能下降或程序崩溃。

   **I/O设备性能**：如果任务涉及磁盘I/O或网络I/O，那么磁盘的读写速度或网络带宽可能成为瓶颈。即使增加线程数，如果I/O设备已经饱和，则无法提升性能，反而可能因为过多的线程竞争I/O资源而降低性能。

   **任务依赖和资源竞争**：如果多个线程需要竞争同一个资源（如共享数据、锁等），那么增加线程数可能会增加锁竞争，导致性能下降。此时，线程数应控制在减少竞争和提高并行度之间找到平衡。

   **外部服务的连接数限制**：例如，数据库连接池的最大连接数、第三方API的速率限制等。如果线程数超过这些限制，额外的线程将无法获得连接或会被限流，从而无法工作。

##### 为什么多线程，什么时候多线程什么时候多进程

多线程的优势：

1. **资源共享**：线程共享相同的内存空间，数据共享更加高效
2. **创建开销小**：线程创建和上下文切换的开销远小于进程
3. **通信简单**：线程间通信可以通过共享内存直接进行，无需复杂的IPC机制
4. **响应性好**：在GUI应用中，多线程可以保持界面响应性
5. **利用多核**：可以充分利用多核CPU的计算能力

多进程的优势：

1. **隔离性好**：进程间有独立的内存空间，一个进程崩溃不会影响其他进程
2. **安全性高**：进程间相互隔离，提供了更好的安全性
3. **简化编程**：不需要复杂的同步机制（对于不共享内存的情况）
4. **扩展性强**：可以跨多台机器分布进程（分布式系统）

场景：

多线程:  

 **I/O密集型任务**：需要处理大量I/O操作（网络、磁盘I/O)  如  ：Web服务器处理并发请求,数据库连接池

**需要共享状态的应用程序** ： GUI应用程序（保持界面响应同时执行后台任务）

多进程：

**需要容错性的系统** ： 一个进程崩溃不影响其他服务
...
具体来说，用一个mutex锁，和两个条件变量notFull、notEmpty，他们用来协调生产者和消费者的执行顺序。

每个线程执行一个runInThread函数，不停的take。
...
负载均衡算法是分布式系统中**将请求 / 任务合理分配到多个服务器节点**的核心策略，目的是避免单点过载、提升系统吞吐量与可用性。

`SO_REUSEPORT` 是 Linux 内核（3.9+ 版本引入）提供的套接字选项，用于实现多线程 / 多进程共享同一端口监听，其负载均衡机制由内核底层实现，核心是通过**哈希分发**和**连接隔离**避免传统模式的锁竞争与惊群效应

内核会对每个新到来的 TCP 连接的**四元组**（源 IP、源端口、目的 IP、目的端口）计算哈希值

- 哈希算法确保同一客户端的连接会被分配到同一个监听套接字（减少乱序问题）。
- 不同客户端的连接会通过哈希均匀分散到不同的监听套接字，实现基础负载均衡。

每个线程 / 进程拥有自己独立的监听套接字（`listen fd`），仅负责处理内核分配给自己的连接，无需与其他线程竞争

- 避免了传统 “单监听套接字 + 多线程竞争 `accept`” 的**惊群效应**（所有线程被唤醒但只有一个能抢到连接）。
- 无需锁同步，每个线程独立处理自己的连接队列，性能损耗极低。
...
核心目标是让开发者像调用本地函数一样调用远程服务器上的函数，屏蔽网络通信的复杂细节。

原理即架构框图

1. **本地调用触发远程请求**：调用方代码中直接调用`clientStub.add(a, b)`（看起来像本地函数），实际触发客户端存根的逻辑。
2. **参数序列化**：客户端存根将参数`a`和`b`（内存中的整数）通过序列化工具（如 Protobuf）转为二进制数据流。
3. **组装请求协议包**：客户端存根将 “服务名（如`CalculatorService`）、函数名（如`add`）、二进制参数、请求 ID” 等信息组装成符合 RPC 协议的数据包（如 HTTP2 帧、自定义 TCP 包）。
4. **发起网络请求**：客户端存根通过网络传输层（如 Socket）建立与服务方的连接，并将请求数据包发送到服务方的 IP 和端口（地址从注册中心获取）。
5. **服务方接收请求**：服务方的网络传输层监听端口，收到请求数据包后，将其传递给服务端存根。
6. **请求协议解析**：服务端存根解析数据包，提取出 “服务名、函数名、二进制参数、请求 ID”。
7. **参数反序列化**：服务端存根将二进制参数通过反序列化工具转为内存中的`a`和`b`。
8. **调用本地业务函数**：服务端存根根据 “服务名 + 函数名” 找到本地实际的`add(a, b)`函数，调用并获取返回值（如`a+b=5`）。
9. **返回值序列化与响应**：服务端存根将返回值`5`序列化，组装成响应数据包（携带请求 ID，用于匹配请求），通过网络传输层返回给调用方。
10. **结果反序列化与返回**：调用方的网络传输层接收响应数据包，传递给客户端存根；客户端存根反序列化得到返回值`5`，并将其返回给调用方的业务代码，完成整个调用。

##### 同步和异步RPC调用

同步即 调用方阻塞线程，直到收到结果

异步即 调用方发起 RPC 请求后，**无需等待响应**，可以立即返回并继续执行后续代码。

异步RPC调用实现方法有，1. 通过回调函数，2.通过future/promise
...
为了降低cpu负载，满足低端cpu较差的情况下仍能10帧运行算法的要求，需要开发模块耗时统计工具，该工具不仅仅是统计模块的耗时，而且需要可以统计各个代码块的耗时。
...
资源的使用一般经历三个步骤a.获取资源 b.使用资源 c.销毁资源，但是资源的销毁往往是程序员经常忘记的一个环节，所以程序界就想如何在程序员中让资源自动销毁呢？c++之父给出了解决问题的方案：RAII，它充分的**利用了C++语言局部对象自动销毁的特性来控制资源的生命周期**。

当我们在函数中声明一个局部对象的时候，会自动调用构造函数进行对象的初始化，当函数执行完成后，自动调用析构函数来销毁对象，整个过程无需人工介入，由操作系统自动完成；于是，很自然联想到，当我们在使用资源的时候，在构造函数中进行初始化，在析构函数中进行销毁
