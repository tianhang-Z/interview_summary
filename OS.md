...
页表基址寄存器   多级页表  获取物理页索引      物理页索引+偏移即实际物理地址
...
![image-20250322152525302](./image/OS_image/image-20250322152525302.png)
...
进程是系统进⾏资源分配和调度的基本单位。  一个进程就是执行的程序和它的上下文（寄存器 内存映射信息-页表 堆栈  打开的文件表 占用的IO资源等   通过PCB保存了进程的很多信息：pid 父进程 ）。

进程：由于**每个进程都有独⽴的内存空间**，创建和销毁进程的开销较⼤。**进程间上下文切换开销大，上下文信息多**。 

线程：是进程内的执⾏单元，  线程共享相同的内存空间，  线程的上下文内容较少（寄存器 线程栈   使用TCB记录信息：tid 优先级

协程：用户态的轻量级线程，由应用程序管理调度。协程共享线程空间
...
在 32 位 Linux 系统里，一个进程的虚拟空间是 4G，内核占1G，用户3G，线程栈大小为1MB：
...
在类 Unix 文件系统（如 ext4）中，目录项通常存储在目录文件中，而实际的文件元数据和文件内容则存储在 `inode` 结构中。
