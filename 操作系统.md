...
**每个CPU都有自己的Cache，三级Cache包含在CPU中**

现代内存中，保存的是页和页表，页大小为4kb页，Cache是为了加速内存的访问。

访问内存前，由地址翻译单元MMU通过查快表和页表获取物理地址，然后根据物理地址访问内存中具体的页。（快表放在一个与Cache类似的高速缓存中）

根据物理地址，先从L1访问，miss之后再向下继续访问。
...
内存虚拟化：虚拟内存，OS内核引入了虚拟内存地址，为每个进程提供了一个独立的虚拟内存空间，使得进程以为自己独占全部内存资源，再通过内存映射，将虚拟内存地址映射到物理内存地址，使得进程之间互不影响。
...
页表基址寄存器保存一级页表的地址，一级页表中记录了二级页表的基址，利用虚拟地址的多个索引结合多级页表，获取物理页索引。
...
多级页表是为了缩小页表所占的内存空间，虚拟内存中相当一部分是没有被使用的，不需要进行虚拟地址映射也不需要页表保存。
...
​	下图为一个elf格式的可执行文件，内部分为多个段，其中记录了段如何映射到运行时内存段。

![image-20250910141513429](./image/操作系统_image/image-20250910141513429.png)
...
程序运行时按需申请和释放内存的过程，是堆(heap)上的内存分配与释放。

内存池对内存的管理方法有：显式空闲链表，隐式空闲链表（在块头部包含块大小、是否分配），分离适配（多个链表）
...
**交换空间（Swap Space）** 是计算机系统中用于扩展物理内存（RAM）的一种虚拟内存机制。当系统的物理内存不足时，操作系统会将部分暂时不用的数据从 RAM内存移动到磁盘上的交换空间，以腾出 RAM 给更需要的进程使用。
...
外部中断是由CPU外部设备或事件引发的，是与当前执行的指令无关的**异步事件**。中断请求通常通过硬件信号线传递给CPU。
...
* 进程：进程是系统进⾏资源分配和调度的基本单位。  一个进程就是执行的程序和它的上下文（寄存器 内存映射信息-页表 堆栈  打开的文件表 占用的IO资源等   通过PCB保存了进程的很多信息：pid 父进程 ）。
  ...
  **进程间上下文切换开销大，上下文信息多**，创建和销毁进程的开销较⼤，需要操作系统介入， 由操作系统调度。

* 线程：是进程内的执⾏单元，是CPU调度的基本单位，  线程的上下文内容较少（寄存器 线程栈   使用TCB记录信息：tid ）
...
ulimit -a 这条命令，查看进程创建主线程时默认分配的栈空间大小
...
如果你只需要对一些**简单类型的数据（如整数）进行访问控制**，并且需要高效的**无锁同步**，可以使用 **原子操作**。`std::atomic` 提供了一组无锁的操作，它可以确保对共享变量的访问是原子性的，不需要使用显式的锁。
