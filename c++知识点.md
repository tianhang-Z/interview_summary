...
==面试简答：⽤于指定变量、指针、引⽤、成员函数、成员变量、返回值、函数参数时的效果==。
...
`volatile` 是 C++ 中的一个关键字，用于**告诉编译器某个变量可能会在程序的控制之外被修改**（例如，硬件、操作系统或其他线程）。
...
1，define在预处理阶段进行字符替换，inline是将内联函数编译完成后，插入调用的地方。
...
2. 尤其是进⾏**下⾏转换时， 具有类型检查**的功能，⽐static_cast更安全。
...
1. **`typeid`操作符** - 获取对象的类型信息
2. **`dynamic_cast`操作符** - 安全地进行向下转型（downcasting）
3. **`std::type_info`类** - 表示类型信息

说明：

- **类型信息结构**：编译器为每个具有虚函数的类生成一个类型信息结构（通常是 `std::type_info` 类型的对象），该结构包含关于类的信息，如类名、基类信息等。
* 类有虚函数，则有虚表；类的对象则有虚指针vptr，虚指针指向虚表；dymamic_cast利用虚指针找虚表 ，通过虚表信息可以获取类型信息( 虚函数表，其中通常包含指向 `std::type_info`对象的指针)
...
**虚函数表（vtable）通常存储在程序的只读数据段（例如 `.rodata`段），从逻辑上讲，它属于全局数据区，但更准确地说是存储在程序的常量数据区域。**
...
使用 `enum class` 可以**避免许多传统枚举类型的名称冲突和隐式转换问题**。
...
2. **内存分配**：
   - 静态内存分配是在程序==编译时确定==的，分配的内存量在程序运行期间是固定的。
...
int main() {
    ...
    MyClass Obj3 = obj2);  // 调用拷贝构造
    MyClass Obj4(obj2);   //调用拷贝构造
...
自定义对象的运算符行为，使得用户定义的类型==（类）可以像内置基本类型一样使用运算符==
...
- **构造过程**：如果传递的是一个临时对象，`push_back`会调**用移动构造函数**来将对象添加到容器中。
