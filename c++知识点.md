...
1，struct表示一组数据，class表示封装了数据和操作的对象
...
C++中的多态性是通过虚函数（virtual function）和虚函数表（vtable）来实现的。
...
静态多态和动态多态 ：**函数重载、模板**是静态多态，编译期检查；**虚函数是动态多态** ，运行期
...
2，分配方式：栈系统自动分配   ；堆手动使用库函数（动态内存分配器 管理空闲空间链表）

3，大小限制：栈的大小是设置好的  一般4M ； 堆扩展 空间很大 1-4G

4，空间连续性：栈连续 ；堆由于申请释放等 有很多碎片 不连续

5，效率：自动的效率高；手动分配效率低，且还有管理空闲空间，肯定慢
...
许多计算机系统对基本数据类型的==合法地址==做出了一些限制 ，要求**某种类型对象的地址必须是某个值 K(通常是 2、 4 或 8)的倍数**。

目的是使数据尽量一次访问得到。

因为寄存器一般是8/ 32 /64位的  

这种对齐限制**简化**处理器和内存系统之间接口的硬件设计。例如，假设一个处理器总是从内存中取 8 个字节， 则地址必须为 8的倍数。如果我们能保证将所有的 double 类型数据的地址对齐成 8 的倍数，那么就可以用一个内存操作来读或者写值了。

==对齐本质是空间换时间==   满足不同平台对合法地址的要求，使数据尽量一次访问得到。

结构体内成员**按照声明顺序存储**，第一个成员地址和整个结构体地址相同。

结构体的大小计算遵循结构体的对齐规则：

1. 成员变量要==对齐到某个数字（对齐数）的整数倍的地址处==。
2. 结构体的总大小为最大对齐数（每个成员变量都有一个对齐数）的整数倍。
3. 如果嵌套了结构体，嵌套的结构体对齐到自己的最大对齐数的整数倍处，==结构体的整体大小就是所有最大对齐数（含嵌套结构体的对齐数）的整数倍==。

对齐数 = 该结构体成员变量自身的大小与编译器默认的一个对齐数的较小值。比如char对齐到1 ，int 对齐到4，等待。
注：【**VS**中默认的值为8、**Linux**环境默认不设对齐数（对齐数是结构体成员自身的大小）】
...
5、**实现原理：** new会先==调用operator new函数==，申请足够的内存（通常底层使用malloc实现，根据类的成员变量类型、是否有虚函数等来计算类大小）。然后==调用类型的构造函数==，初始化成员变量，最后返回自定义类型指针。delete==先调用析构函数==，然后==调用operator delete函数释放内存==（通常底层使用free实现）。
...
3. **插入删除的效率**
   vector：尾部进⾏插⼊/删除操作较快，但在中间或头部进⾏插⼊/删除可能会涉及⼤量元素的移动。
   
   list:  在任意位置进⾏插⼊/删除操作都是常数时间复杂度。  
...
都是C++的关联容器，底层都是采⽤红⿊树实现，默认升序排列。  logn的复杂度
...
td::unique_ptr<MyClass> ptr2 = std::move(ptr1); // 转移所有权  
```
...
- **防止窄化转换**：列表初始化不允许窄化转换（如将 `double` 转换为 `int` 时丢失精度）。
...
- **避免歧义**：在某些情况下，列表初始化可以避免函数声明和对象初始化之间的歧义。
