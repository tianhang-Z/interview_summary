...
**火焰图（Flame Graph）** 是将 `perf` 数据可视化的主流方式， 火焰图由 Brendan Gregg 开源的脚本生成

perf top   trace record  stat

* 定位 CPU 使用率高的瓶颈：某 CPU 使用率长期 100%，需找到消耗 CPU 的函数。 

* 诊断缓存未命中问题  ：某计算密集型程序运行缓慢，`perf stat` 显示缓存未命中率高达 40%。    （数组访问不符合局部性原理时

* 追踪系统调用开销： 某程序执行时间长，但 CPU 使用率低，怀疑是系统调用阻塞。

#### valgrind

* 原理：**动态二进制插桩**
  - 当使用 `valgrind` 命令运行程序时，Valgrind 会先**启动自己的内核**（core），然后将目标程序加载到这个内核中。
  - **目标程序并非直接在 CPU 上运行，而是由 Valgrind 内核控制其执行流程**。
  - Valgrind 会将目标程序的机器码（x86、amd64 等）**翻译成一种中间表示**（Intermediate Representation, IR）。
  - 在翻译过程中，Valgrind 会根据所使用的工具（如内存检测工具 Memcheck）**插入额外的代码**（instrumentation），用于监控程序的内存访问、函数调用等行为。
  - **翻译后的 IR 代码在 Valgrind 模拟的 CPU 环境中执行。**
  - 插入的监控代码会实时检查程序行为（如内存访问是否越界、释放后的内存是否被使用等），一旦发现问题就记录并报告。

* 主要工具

  Valgrind 包含多个工具，其中最常用的是 **Memcheck**（内存错误检测），可检测以下错误

  - 使用未初始化的内存

  - 读取 / 写入已释放的内存

  - 读取 / 写入超出 malloc/new 分配的内存范围

  - 内存泄漏（未释放不再使用的内存）

  - 使用 `free` 或 `delete` 多次释放同一内存块

  - `malloc`/`free` 与 `new`/`delete` 混用
...
int main() {
  ...
  cout << "ptr2-ptr1 " << (char*)ptr2 - (char*)ptr2 << endl;
...
![image-20250927214812422](./image/面经和笔试_image/image-20250927214812422.png)

* **`sizeof`是在编译期计算**的,  **类型大小是固定的**,  结构体/类的大小由成员变量和内存对齐规则决定，编译时也能计算。
* **基类指针指向派生类时，编译器会自动调整指针值，使其指向派生类对象内部的基类子对象部分**。这是 C++ 多态和继承机制的核心行为之一。、
  * `ptr1`直接指向 `Derived`对象的起始地址（`Base1`子对象）。
  * `ptr2`不能直接指向 `Derived`的起始地址，因为 `Base2`子对象在 `Base1`之后。
  * **编译器会自动调整指针**，使其指向 `Base2`子对象的起始位置（偏移 `sizeof(Base1)`字节）。

### typeid

`typeid`是 C++ 中的一个运算符，用于在运行时获取对象的类型信息。它是 RTTI（Run-Time Type Information，运行时类型识别）机制的一部分。
...
int main() {
  ...
  std::cout << "i 的类型是: " << typeid(i).name() << std::endl;
  std::cout << "d 的类型是: " << typeid(d).name() << std::endl;

  auto result = 10 + 3.14;
  std::cout << "10 + 3.14 的类型是: " << typeid(result).name() << std::endl;

  Base* b = new Derived();
  std::cout << "b 指向的实际类型是: " << typeid(*b).name() << std::endl;
